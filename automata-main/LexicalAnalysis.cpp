...
DiagramElements DiagramBuilder::buildExampleDiagram(QGraphicsScene* scene)
{
    ...
    // Shared 'f' transition for def, elif, and if keywords
    elements.transitions["shared_f"] = DrawHelper::createArrow(scene, s3, f4, "f");


    // Shared 'e' and 'l' transitions for elif and else keywords
    ...
    // 'if' shares the 'f' transition with def and elif
    ...
    //elements.transitions["while_e"] = DrawHelper::createArrow(scene, s11, f4, "e");
...
QList<AnimationStep> LexicalAnalysisTab::getAnimationSteps(const QString& token, const QString& type)
{
    ...
    if (type == "Keyword") {
        ...
        // Animate each character of the number individually
        bool foundDot = false;
        ...
        for (int i = 0; i < token.length(); ++i) {
            ...
            if (c == '.') {
                // Transition from f1 to s1 on dot
                ...
                if (i == 0) {
                    ...
                    // Integer part digits after first: f1 -> f1 (self-loop)
                    ...
                    // First decimal digit: s1 -> f2
                    steps.append({"s1", "f2", "number_decimal"});
                    inDecimalPart = false;
                }
                else {
                    // Subsequent decimal digits: f2 -> f2 (self-loop)
        ...
        // Animate each character of the identifier individually
        for (int i = 0; i < token.length(); ++i) {
            if (i == 0) {
                // First character: s0 -> f3
                steps.append({"s0", "f3", "identifier"});
            } else {
                // Subsequent characters: f3 -> f3 (self-loop)
